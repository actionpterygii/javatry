...
public class Step03DataTypeTest extends PlainTestCase {
    ...
    public void test_datatype_basicType() {
        ...
        land = bonvo.getMonthValue(); // 9 x-> 10
        //        log(land);
        land--; // 8 x-> 9
        if (dstore) {
            BigDecimal addedDecimal = amba.add(new BigDecimal(land)); // 17.4 x-> 18.4
            sea = String.valueOf(addedDecimal); // 17.4 x-> 18.4
        }
        log(sea); // your answer? => // 17.4 x-> 18.4
    }
    ...
    public void test_datatype_primitive() {
        ...
        if (dohotel && dstore >= piari) {
            ...
            sea = (byte) land; // 127
            if (amba == 2.3D) { // true
                sea = (byte) amba; // 127 x-> 2
            }
        }
        if (dstore > piari) { //true x-> false
            sea = 0; // 0
        }
